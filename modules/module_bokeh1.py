...
class Module(pyl.Model):
    ...
    def update_simdata(self):
        ...
        self.widget_sim["TextInput_iteration"].value = str(self.moduleData["/simulation/iteration"])
    ...
    def modify_doc(self, doc):
        ...
        self.data = []
        for i in range(5):
            self.data.append(ColumnDataSource({'x': np.arange(200)*-1*time_step, 'y': np.zeros(200)}))
        # Create plottedlines
        self.lines = []
        ...
        self.signal_type = "sine"
        self.widget_1["radio_button"] = RadioButtonGroup(labels=["Sine", "Block", "Triangle"], active=0)
        self.widget_1["radio_button"].on_click(self.on_click_radio_button)
        self.widget_1["widgetbox"] = widgetbox(self.widget_1["radio_button"], self.widget_1["Button_pause"])
