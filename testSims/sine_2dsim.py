...
class MyScheduler(pyl.Scheduler):
    ...
    def create_sim_data(self):
        ...
        self.simData["/2dsine_generator/outputs/signal"] = []
        elements = self.simData["/2dsine_generator/outputs/signal/dim1"]*self.simData["/2dsine_generator/outputs/signal/dim2"]
        dtype = np.int64
        for i in range(2):
            base_ptr = mp.RawArray(dtype, elements)
            arr = np.frombuffer(base_ptr, dtype)
            arr[:] = np.array(np.zeros(elements), dtype)
            self.simData["/2dsine_generator/outputs/signal"].append(base_ptr)
            self.simData["/2dsine_generator/outputs/signal/type"] = dtype
        ...
        self.simData["/user/inputs/2dscope"] = ["2dsine_generator"]
        #self.simData["/user/inputs/scope2"] = ["adder", "low_pass"]

        
# Simulator Input Variables
script_info = [
                 ["2dsine_generator",      "../modules/module_sine_generator.py"],
...
connectivityMatrix ={   "/adder/inputs/signal1" : "/noise_generator/outputs/signal",
                        ...
                        "/2dscope/inputs/signal" : "/user/inputs/2dscope",
